...
public:
    DiaComException(const char* message, HRESULT result)
        : DiaSymbolMasterException{message}, m_result{result} {};
...
operator<<(std::ostream& os, const DiaSymbolMasterException& exception)
{
...
static inline std::ostream& operator<<(std::ostream& os,
                                       const DiaComException& exception)
{
    os << "DiaComException[" << std::hex << exception.getResult() << std::dec
